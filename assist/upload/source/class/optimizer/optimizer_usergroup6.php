 ...
 *
 *      $Id: optimizer_usergroup6.php 33594 2013-07-12 07:38:33Z jeffjzhang $
 */
...
class optimizer_usergroup6 {
	...
	public function check() {
		$usergroup = C::t('common_usergroup')->fetch(6);
		$usergroupfield = C::t('common_usergroup_field')->fetch(6);
		if(!$usergroup['allowsendpm'] && !$usergroupfiled['allowposturl'] && !$usergroupfield['allowgroupposturl'] && !$usergroupfield['allowpost'] && !$usergroupfield['allowreply'] && !$usergroupfiled['allowdirectpost'] && !$usergroupfield['allowgroupdirectpost']) {
			$return = array('status' => 0, 'type' =>'none', 'lang' => lang('optimizer', 'optimizer_usergroup6_no_need'));
		} else {
			$option = array(
			...
			$return = array('status' => 1, 'type' =>'header', 'lang' => lang('optimizer', 'optimizer_usergroup6_need', array('desc' => implode(',', $desc))));
		}
		return $return;
	}
