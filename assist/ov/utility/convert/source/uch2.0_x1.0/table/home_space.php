...
while ($space = $db_source->fetch_array($query)) {
	...
	if($setarr) {
		...
		$db_target->query("UPDATE {$newpre}common_member SET $updatesql WHERE uid='$space[uid]'");
	}
	...
	if(empty($newspace['notifications'])) $setarr['notifications'] = $space['notenum'];
	if(empty($newspace['myinvitations'])) $setarr['myinvitations'] = $space['myinvitenum'];
	if(empty($newspace['pokes'])) $setarr['pokes'] = $space['pokenum'];
	if(empty($newspace['pendingfriends'])) $setarr['pendingfriends'] = $space['addfriendnum'];
